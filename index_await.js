...
var scraper = async () => {
  try {
    ...
    var final = await page.evaluate(() => {
      ...
      console.log(ex[1].innerText.trim());
    });

    // let ideaNames = await page.$$("#settings");
    // console.log(
    //   ideaNames[0].$eval("thead", el => {
    //     console.log(el);
    //   })
    // );

    // for (const ideas of ideaNames) {
    //   ideas.$eval("#settings>thead >tr:nth-child(1)>td", el => {
    ...
    //     let ideaName = document.querySelectorAll(`.modal-body`);
    //     for
